...
async function getDeprecatedApmIndices(callWithRequest, request, indexPatterns = []) {
    ...
    return Object.keys(indices).reduce((deprecations, index) => {
        if (exports.isLegacyApmIndex(index, indexPatterns, indices[index].mappings)) {
            deprecations.push({
                level: 'warning',
...
exports.isLegacyApmIndex = (indexName, apmIndexPatterns = [], mappings) => {
    const clientVersion = new semver_1.SemVer(lodash_1.get(mappings, '_meta.version', '0.0.0'));
    if (clientVersion.compareMain('7.0.0') > -1) {
        return false;
    }
    const find = apmIndexPatterns.find(pattern => {
        return minimatch_1.default(indexName, pattern);
    });
    return Boolean(find);
};
// source: https://github.com/elastic/apm-integration-testing/blob/master/tests/server/test_upgrade.py
exports.apmReindexScript = `
  // add ecs version
